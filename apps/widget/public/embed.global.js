...
(() => {
  (function () {
    ...
    let m = c.getAttribute("data-organization-id"),
      f = c.getAttribute("data-bot-id"),
      u = c.getAttribute("data-position") || "bottom-right";
    if (!m || !f) {
      console.error(
    ...
    let b = "http://localhost:3001",
      y = "chatify_visitor_id",
      h = "chatify_visitor_id_createdAt",
      I = "#6366f1";
    function T(t) {
      let e = new Date(t);
      return Date.UTC(
    ...
    function _(t) {
      if (!t.startsWith("visitor_")) return null;
      let e = t.split("_");
    ...
    function E(t, e) {
      if (!Number.isFinite(t) || t > e) return !0;
      let a = T(t);
      return e >= a;
    }
    function P(t) {
      return `visitor_${t}_${Math.random().toString(36).substr(2, 9)}`;
    }
    function A() {
      let t = Date.now(),
      ...
      if (!o && e) {
        ...
        n &&
          Number.isFinite(n) &&
          ((o = n), localStorage.setItem(h, String(o)));
      }
      if (!e || !o || E(o, t)) {
        let n = P(t);
        return (
    ...
    function S(t) {
      return t.startsWith("top") ? "top: 80px" : "bottom: 80px";
    }
    function v() {
      return `
    ...
    function x() {
      ...
      ((t.id = `chatify-widget-${m}`),
        t.setAttribute("data-chatify-widget", "true"));
      let e = R(u);
      (t.style.setProperty("position", "fixed", "important"),
        t.style.setProperty(e.horizontal, e.horizontalValue, "important"),
        t.style.setProperty(e.vertical, e.verticalValue, "important"),
      ...
      let a = t.attachShadow({ mode: "open" }),
        o = L(u),
        n = S(u),
    ...
    `),
      ...
      ((i.id = "chatify-button"),
        (i.innerHTML = v()),
        i.setAttribute("aria-label", "Open chat widget"),
        i.setAttribute("aria-expanded", "false"),
        a.appendChild(i));
      let l = document.createElement("div");
      l.id = "chatify-container";
      let s = document.createElement("iframe");
      ((s.id = "chatify-iframe"),
        (s.src = `${b}/widget?orgId=${m}&botId=${f}&visitorId=${A()}`),
        (s.allow = "camera; microphone"),
        s.setAttribute("loading", "lazy"),
        s.setAttribute("title", "Chatify Chat Widget"),
        l.appendChild(s),
        a.appendChild(l));
      let g = !1,
        d = I;
      function z(p) {
        ...
        let r = `${Math.max(0, p)}px`;
        (l.style.setProperty("border-radius", r, "important"),
          s.style.setProperty("border-radius", r, "important"));
      }
      function O() {
        ((g = !0),
          ...
          i.setAttribute("aria-label", "Close chat widget"));
      }
      function C() {
        ((g = !1),
      ...
      (i.addEventListener("click", k),
        window.addEventListener("message", (p) => {
          let V = new URL(b).origin;
          if (p.origin !== V) return;
          let r = p.data;
          (r.type === "widget:close" && C(),
            r.type === "widget:ready" &&
              r.data &&
              (r.data.primaryColor &&
                ((d = r.data.primaryColor),
                i.style.setProperty("background", d, "important")),
              r.data.cornerRadius != null && z(r.data.cornerRadius)),
            r.type === "widget:config" &&
              r.data &&
              (r.data.primaryColor &&
                ((d = r.data.primaryColor),
                i.style.setProperty("background", d, "important")),
